...
library(climate)
ns = nearest_stations_ogimet(country ="United+Kingdom",point = c(-4, 56), no_of_stations = 50, add_map = TRUE)
head(ns)
...
Creating a wind rose of Svalbard in 2018 with external package: **openair**
...
``` {r windrose_plot, echo=F,include=TRUE,fig.width=7, fig.height=3, fig.fullwidth=TRUE}
windRose(mydata = df, ws = "ws", wd = "dir", type = "season", paddle = FALSE,
         main = "Svalbard Lufthavn (2018)", ws.int = 3, dig.lab = 3, layout = c(4, 1))
```
...
```{r sonda, fig.width=7, fig.height=7, fig.fullwidth=TRUE}
library(climate)
...
RadioSonde::plotsonde(df, winds = FALSE, title = "2020-03-23 00UTC (LEBA, PL)",
                      col = c("red", "blue"), lwd = 3)
```

### Example 4
Preparing a annual summary of air temperature and precipitation, processing with **dplyr** and plot by **climatol**
...
The annual means of maximum flow can be also presented on the map using the **tmap** package and boarders of Poland take

```{r, eval=T,warning=F, fig.width=7, fig.height=6, fig.fullwidth=TRUE}
library(sf)
library(tmap)
library(rnaturalearthdata)
...
head(ns)
>>>>>>> cfd9b7041ee412a8c874323c1d9497de2c91db9f
```
