test_that("meteo_ogimet_synop stops when neither station nor country_name is given", {
  expect_error(
    meteo_ogimet_synop(allow_failure = FALSE),
...
test_that("meteo_ogimet_synop station mode Date column is POSIXct UTC", {
  if (!curl::has_internet()) return(invisible(NULL))
  skip_on_cran()
...
test_that("meteo_ogimet_synop station mode handles allow_failure gracefully", {
  ...
  expect_no_error(
    meteo_ogimet_synop(station = 9999999, date = c("2009-12-01", "2009-12-02"),
                       allow_failure = TRUE)
  )
})

test_that("meteo_ogimet_synop station mode source column contains SYNOP strings", {
  if (!curl::has_internet()) return(invisible(NULL))
  skip_on_cran()
...
test_that("meteo_ogimet_synop country mode Date column is POSIXct UTC", {
  if (!curl::has_internet()) return(invisible(NULL))
  skip_on_cran()
