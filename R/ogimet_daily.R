...
ogimet_daily_bp = function(date = date,
  ...
  mindate = min(as.Date(date))
  maxdate = max(as.Date(date))
  # construct sequence of period (one period for each month)
  # dates = end of month
  dates = seq.Date(as.Date(format(mindate,"%Y/%m/01"),format="%Y/%m/%d"), 
                   maxdate,
                   by = "month") 
  dates = dates - 1
  dates = unique(c(dates, maxdate))
  dates = dates[-1]
  ...
  for (station_nr in station) {
    ...
    for (i in length(dates):1) {
      ...
      ndays = day
      linkpl2 = paste("https://www.ogimet.com/cgi-bin/gsynres?lang=en&ind=", station_nr, "&ndays=",ndays,"&ano=", year, "&mes=", month, "&day=", day, "&hora=", hour,"&ord=REV&Send=Send", sep = "")
     # if (month == 1) linkpl2 = paste("https://www.ogimet.com/cgi-bin/gsynres?lang=en&ind=", station_nr, "&ndays=32&ano=", year, "&mes=", month, "&day=", day, "&hora=", hour, "&ord=REV&Send=Send", sep = "")
      ...
      if (is.null(test) ) {
        ...
        break # continue with next iteration in the loop for station
      } 
      
      # all columns are empty:
      if (all(is.na(test[2, ]))) {
        warning("no values in column names")
        next # continue to next period
      } 
      ...
      # Carefull : mistake if period straddling two years!!!
